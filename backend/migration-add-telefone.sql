...
ALTER TABLE tools.users ADD COLUMN IF NOT EXISTS telefone VARCHAR(20);

-- Atualizar usuário admin existente com telefone padrão
UPDATE tools.users SET telefone = '(83) 2101-1000' WHERE username = 'admin' AND telefone IS NULL;

-- Verificar se a migração foi aplicada com sucesso
SELECT 'Migração aplicada com sucesso!' as status;
SELECT username, email, telefone FROM tools.users;